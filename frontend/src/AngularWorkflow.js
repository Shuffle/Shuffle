...
const referenceUrl = window.location.origin+"/functions/webhooks/"
...
const AngularWorkflow = (props) => {
	...
	const TriggersView = () => {
		...
		var triggers = [{
			...
			"description": "Realtime HTTP trigger",
	...
	const appApiView = Object.getOwnPropertyNames(selectedAction).length > 0 && Object.getOwnPropertyNames(selectedApp).length > 0 ? 
		<div style={appApiViewStyle}>
			<div style={{display: "flex", height: "40px", marginBottom: 30}}>
				...
				<div style={{flex: "1"}}>
					<Button disabled={selectedAction.id === workflow.start} style={{marginTop: "15px",}} color="primary" variant="outlined" onClick={(e) => {
						defineStartnode(e)	
			...
			<div style={{marginTop: "20px"}}>
				Environment:
				<Select
					value={selectedActionEnvironment.Name}
					PaperProps={{
						style: {
							backgroundColor: inputColor,
						}
			...
			{/* FIXME authenticat
			<div style={{marginTop: "20px"}}>
				<Button fullWidth style={{margin: "auto", marginTop: "10px",}} color="primary" variant="outlined" onClick={() => setAuthenticationModalOpen(true)}>
		...
		<div style={{color: "white"}}>
			<div style={{display: "flex", borderTop: "1px solid rgba(91, 96, 100, 1)"}}>
				...
				<CytoscapeComponent 
					...
					minZoom={0.35}
					maxZoom={2.00}
					style={{width: bodyWidth-leftBarSize-1, height: bodyHeight-appBarSize-1, backgroundColor: surfaceColor}} 
					stylesheet={cystyle}
	...
	const AuthenticationData = () => {
		...
		if (currentAuth.scheme.toLowerCase() === "bearer") {
			return <div>
				NOT IMPLEMENTED
				Insert your bearer token for {selectedApp.name}
				<TextField
					style={{backgroundColor: inputColor}} 
					InputProps={{
						style:{
