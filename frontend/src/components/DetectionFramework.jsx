					...
					<Paper style={{width: 250, maxHeight: 400, overflow: "hidden", zIndex: 12500, padding: 25, paddingRight: 35, backgroundColor: theme.palette.surfaceColor, border: "1px solid rgba(255,255,255,0.2)", position: "absolute", top: 50, left: 50, }}>
						...
						{selectionOpen ? 
							isCloud ? 
								...
								/>
								: 
							appList === undefined || appList === null || appList.length === 0 ? 
								<CircularProgress />
							:
              appList.map((data, index) => {
								console.log(data.name, newSelectedApp.name)
									...
									<span key={index} style={{ zIndex: 10 }}>
										<IconButton
											style={{
												...
												margin: 0,
												padding: 12,
												border: newSelectedApp.name === data.name ? "2px solid #f86a3" : null,
											}}
											onClick={() => {
												if (newSelectedApp.name !== data.name) {
